#!/usr/bin/env bash
# K8s-adapted copy of scripts/test-security-saga.sh (STR-145). No
# `docker compose exec`/`logs` calls to translate -- it only talks to the
# stack over HTTP through the gateway, which already works unmodified
