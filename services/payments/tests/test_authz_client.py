...
async def test_check_allows_when_opa_returns_true():
    route = respx.post("http://opa.invalid/v1/data/payments/allow").mock(
...
async def test_check_denies_when_opa_returns_false():
    respx.post("http://opa.invalid/v1/data/payments/allow").mock(
...
async def test_check_fails_closed_when_sidecar_unreachable():
    respx.post("http://opa.invalid/v1/data/payments/allow").mock(side_effect=httpx.ConnectError("connection refused"))
...
async def test_check_fails_closed_on_5xx():
    respx.post("http://opa.invalid/v1/data/payments/allow").mock(return_value=httpx.Response(500))
