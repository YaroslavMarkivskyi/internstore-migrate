...
class ObjectStorageClient:
    ...
    Two internal boto3 clients, not one: `_client` talks to `endpoint` for
    put_object, but presigned URLs must be *signed* against the host a
    ...
    `key_prefix` namespaces every key this instance writes/signs -- catalog
    and chat share one physical GCS bucket and rely on this prefix
    ...
    bare keys and never see the prefix -- it's applied here, once."""
