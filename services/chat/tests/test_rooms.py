...
async def test_lazy_room_creation_on_first_connect(app, ws_client, client, customer_token, admin_token):
    ...
    response = await client.get("/rooms", headers={"x-internal-token": admin_token})
...
async def test_delete_room_removes_room_and_messages(app, ws_client, client, customer_token, admin_token):
    ...
    delete_response = await client.delete(f"/rooms/{CUSTOMER_ROOM}", headers={"x-internal-token": admin_token})
    assert delete_response.status_code == 204

    messages_response = await client.get(
        f"/rooms/{CUSTOMER_ROOM}/messages", headers={"x-internal-token": admin_token}
    )
    assert messages_response.status_code == 404


async def test_non_admin_cannot_list_rooms(client, customer_token):
    response = await client.get("/rooms", headers={"x-internal-token": customer_token})
