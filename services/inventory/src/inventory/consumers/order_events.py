...
Handler = Callable[[AsyncSession, dict, float], Awaitable[None]]
...
async def handle_order_created(session: AsyncSession, event_id: uuid.UUID, payload: dict, ttl_seconds: float) -> None:
...
async def handle_payment_confirmed(session: AsyncSession, event_id: uuid.UUID, payload: dict, ttl_seconds: float) -> None:
    if await _already_processed(session, event_id):
        return
    session.add(ProcessedEvent(event_id=event_id))
...
def make_dispatch(session_factory: async_sessionmaker, ttl_seconds: float) -> Callable[[dict], Awaitable[None]]:
    async def dispatch(envelope: dict) -> None:
        handler = HANDLERS.get(envelope.get("event_type", ""))
        if handler is None:
            return
        async with session_factory() as session:
            await handler(session, uuid.UUID(envelope["event_id"]), envelope.get("payload", {}), ttl_seconds)
