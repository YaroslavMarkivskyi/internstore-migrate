...
from inventory.observability import record_kafka_lag
...
async def run_consumer_loop(bootstrap_servers: str, topic: str, group_id: str, dispatch: Dispatch) -> None:
    ...
    non-JSON message on order-events wedged this loop with no error in the
    logs and no recovery even across a service restart, since the next
    consumer hit the exact same unparseable message at the same offset).
