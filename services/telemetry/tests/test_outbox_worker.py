...
from telemetry.models import OutboxEvent
from telemetry.outbox_worker import publish_pending_events
...
async def test_publish_pending_events_publishes_and_marks_published(client, session):
    session.add(OutboxEvent(event_type="TemperatureThresholdViolated", payload={"stock_id": "abc"}))
    ...
    assert producer.sent == [("telemetry-events", "TemperatureThresholdViolated", {"stock_id": "abc"})]
...
async def test_publish_pending_events_skips_already_published(client, session):
    session.add(
        OutboxEvent(event_type="TemperatureThresholdViolated", payload={}, published_at=datetime.now(timezone.utc))
    )
    await session.commit()
