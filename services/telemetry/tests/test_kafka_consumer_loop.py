...
import telemetry.kafka as kafka_module
from telemetry.kafka import run_consumer_loop
...
async def test_unparseable_message_is_skipped_and_committed(monkeypatch):
    messages = [
        ...
        FakeMessage(b'{"event_type": "ProductCreated", "payload": {}}', offset=1),
    ]
    ...
    await run_consumer_loop("kafka:9092", "catalog-events", "test-group", dispatch)

    assert dispatched == [{"event_type": "ProductCreated", "payload": {}}]
    assert fake_consumer.committed_offsets == [0, 1]


async def test_dispatch_failure_is_not_committed_and_reraises(monkeypatch):
    messages = [FakeMessage(b'{"event_type": "ProductCreated", "payload": {}}', offset=0)]
    fake_consumer = FakeConsumer(messages)
    monkeypatch.setattr(kafka_module, "AIOKafkaConsumer", fake_consumer)
