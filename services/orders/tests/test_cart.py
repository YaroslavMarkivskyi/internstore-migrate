...
async def test_get_empty_cart(client, customer_token):
    resp = await client.get("/cart", headers=customer_token)
...
async def test_carts_are_isolated_by_owner(client, customer_token, admin_token, guest_token):
    product_id = str(uuid.uuid4())
    await client.post(
        "/cart", json={"product_id": product_id, "quantity": 1}, headers=customer_token
    )

    admin_resp = await client.get("/cart", headers=admin_token)
    assert admin_resp.json() == {"items": []}

    guest_resp = await client.get("/cart", headers=guest_token)
