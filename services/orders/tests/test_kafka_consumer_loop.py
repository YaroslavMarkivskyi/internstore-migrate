...
import orders.kafka as kafka_module
from orders.kafka import run_consumer_loop
...
async def test_unparseable_message_is_skipped_and_committed(monkeypatch):
    messages = [
        ...
        FakeMessage(b'{"event_type": "StockReserved", "payload": {}}', offset=1),
    ]
    ...
    await run_consumer_loop("kafka:9092", "inventory-events", "test-group", dispatch)

    assert dispatched == [{"event_type": "StockReserved", "payload": {}}]
    assert fake_consumer.committed_offsets == [0, 1]


async def test_dispatch_failure_is_not_committed_and_reraises(monkeypatch):
    messages = [FakeMessage(b'{"event_type": "StockReserved", "payload": {}}', offset=0)]
    fake_consumer = FakeConsumer(messages)
    monkeypatch.setattr(kafka_module, "AIOKafkaConsumer", fake_consumer)
