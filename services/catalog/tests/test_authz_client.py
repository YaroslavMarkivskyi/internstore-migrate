...
async def test_check_allows_when_opa_returns_true():
    route = respx.post("http://opa.invalid/v1/data/catalog/allow").mock(
    ...
    assert b'"role":"admin"' in body


@respx.mock
async def test_check_denies_when_opa_returns_false():
    respx.post("http://opa.invalid/v1/data/catalog/allow").mock(return_value=httpx.Response(200, json={"result": False}))
...
async def test_check_fails_closed_when_sidecar_unreachable():
    respx.post("http://opa.invalid/v1/data/catalog/allow").mock(side_effect=httpx.ConnectError("connection refused"))
...
async def test_check_fails_closed_on_timeout():
    respx.post("http://opa.invalid/v1/data/catalog/allow").mock(side_effect=httpx.TimeoutException("timed out"))
...
async def test_check_fails_closed_on_5xx():
    respx.post("http://opa.invalid/v1/data/catalog/allow").mock(return_value=httpx.Response(500))
